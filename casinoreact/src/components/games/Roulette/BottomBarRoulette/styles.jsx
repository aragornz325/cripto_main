...
export const Container = styled.div`
	...
	/* box-shadow: inset 10px 10px 17px rgba(46, 32, 32, 0.25); */
...
export const Button = styled.button`
	...
	border: .15vw solid ${props => props.disabled ? "#fff" : props.color};
	border-radius: 100%;
	height: 4vw;
...
&:hover {
	background: ${props => !props.disabled && props.color};
	box-shadow: ${props => props.disabled ? "none" : `0px 0px 15px ${props.color}`};
	font-size: ${props => !props.disabled && '.9vw'};
	color: ${props => props.disabled ? props.color : "#000"};
	
	${props => props.text === "Call" && !props.disabled ? ` &::before {
		content: "${props.price}";
		...
		border: .15vw solid ${props.color};
		box-shadow: 0px 0px 15px ${props.color}};
	}
` : null}`;
...
export const ContainerRaise = styled.div`
	...
	width: 50%;
	padding: 0 1%;
	filter: ${props => !props.disabled ? "none" : "grayscale(100%)"}; 
...
export const MoneyStyle = styled.img`
    width: 58.63px;
    height: 58.63px;
...
export const ContainerFirstButtons = styled.div`
display: flex;
align-items: center;
width: 40%;
padding: 0 1%;
